...
Follow steps 1-5 from the [previous tutorial](/build/build-with-ai/build-with-goat/token-swap-agent) to clone the GOAT repository, navigate to the typescript directory, install dependencies, build the project, and go to the example directory: examples/by-use-case/evm-mint-nft.
