...
description: Build AI agents on Celo that pay, transact, and coordinate onchain with stablecoins
---
...
For the current token and adapter addresses on Mainnet and Celo Sepolia, see [Fee Currencies](/tooling/contracts/fee-currencies) — generated from the on-chain `FeeCurrencyDirectory` allowlist.
...
Agents transacting on behalf of your app can also carry an [attribution tag](/build-on-celo/attribution-tags), so the on-chain activity they generate is credited back to you — useful for tracking usage across MiniPay or your own analytics.
