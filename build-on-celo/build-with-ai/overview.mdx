...
[Celina](/build-on-celo/build-with-ai/mcp/celina) is a third-party stack that gives an LLM read, prepare, and execute access to Celo mainnet — balances, transfers, DeFi, governance, staking, and Self Agent ID — through an SDK, a local MCP server, a hosted MCP server, and a REST API.
