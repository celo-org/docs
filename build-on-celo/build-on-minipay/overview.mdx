...
MiniPay maps phone numbers to addresses through [SocialConnect](/build-on-celo/build-on-socialconnect) and ODIS. To look a number up you act as an *issuer*: an account that has verified the user owns the number (for example by SMS) and that holds ODIS quota.
...
import { celoSepolia } from "viem/chains";                 // Celo Sepolia (11142220)
import { privateKeyToAccount } from "viem/accounts";
import { OdisUtils } from "@celo/identity";
import { AuthSigner, OdisContextName } from "@celo/identity/lib/odis/query";

const viemClient = createWalletClient({
  account: privateKeyToAccount(process.env.ISSUER_PRIVATE_KEY as `0x${string}`),
  transport: http(),
  chain: celoSepolia,
});
const issuerAddress = viemClient.account.address;

// 1. Authenticate with ODIS as the issuer
...
// 2. Check quota; top up by paying the OdisPayments contract if remainingQuota < 1
const { remainingQuota } = await OdisUtils.Quota.getPnpQuotaStatus(issuerAddress, authSigner, serviceContext);

// 3. Derive the obfuscated identifier for the phone number
...
// 4. Look up the addresses attested for it by the issuers you trust
...
Use `OdisContextName.MAINNET` and `chain: celo` in production. The MiniPay reference for this is [Phone number lookup](https://docs.minipay.xyz/technical-references/phone-number-lookup.html).
