...
og:description: A quickstart guide for building on MiniPay and Celo.
...
<Note>
MiniPay is only available on Celo and Celo Sepolia Testnet. Other blockchain networks are not supported.
</Note>
...
```
- Follow the [Quickstart Guide](/build/quickstart) for a step-by-step tutorial.
...
For a more in depth guide, check out the official [ngrok setup](/build/build-on-minipay/prerequisites/ngrok-setup).
