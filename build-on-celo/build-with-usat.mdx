...
USA₮ and USDT are separate assets from different issuers with different trust profiles:
...
Both are available on Celo Mainnet. Use USDT for general stablecoin flows; use USA₮ when compliance, regulatory clarity, or institutional credibility matters to your use case.
...
## Distribution via Self + Google Cloud

Alongside launch, [Self](https://self.xyz/) and Google Cloud built a mainnet faucet that distributes USA₮ to verified real humans. Users verify their identity through the Self app using a biometric passport or national ID. Self confirms humanity (including OFAC sanctions clearance) using zero-knowledge proofs — no raw personal data is transmitted.

This is the first time a regulated USD stablecoin has been distributed through a privacy-preserving identity layer at the protocol level.

Builders can integrate the same faucet pattern using the [Self SDK](https://docs.self.xyz/).

## Builder Note — Wallet Display

Some wallets derived from Valora may currently display `priceUsd: NaN` for USA₮. This is a display issue only — transactions work correctly. Price oracle coverage for the Celo contract address is still expanding.
...
**Compliant remittance** — Use USA₮ as the regulated on-ramp leg, then convert to a local stablecoin via Mento (USDm, BRLm, KESm, NGNm, and more), Ripio wFIAT (wBRL, wARS, wMXN, wCOP), or VNX (VEUR, VGBP, VCHF) depending on the destination corridor.
