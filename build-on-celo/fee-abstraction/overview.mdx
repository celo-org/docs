...
The full up-to-date list of whitelisted fee currencies — including token and adapter addresses — is automatically maintained at [Token Contracts](/tooling/contracts/token-contracts).

Tokens with non-18 decimals (e.g. USDC, USD₮, USA₮ with 6 decimals) require an adapter contract. Use the adapter address — not the token address — in the `feeCurrency` field. See the adapter table in [Using Fee Abstraction](/build-on-celo/fee-abstraction/using-fee-abstraction#adapter-addresses).
