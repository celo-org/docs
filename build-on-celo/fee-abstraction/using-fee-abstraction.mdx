...
The current adapter and token addresses for each network are listed on the [Fee Currencies](/tooling/contracts/fee-currencies) page, which is generated from the on-chain `FeeCurrencyDirectory` allowlist.
