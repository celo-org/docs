...
sidebarTitle: "Running Oracles"
og:description: How to run an oracle for Mento, the stability protocol.
---
...
Oracles work by running a client that fetches the price from centralized exchanges (CEX) and pushes those prices on-chain by calling `SortedOracles.report(address token, uint256 value, address lesserKey, address greaterKey)`. SortedOracles is a [Celo Core Contract](/contribute-to-celo/release-process/smart-contracts).
...
- A dedicated full node running in its own VM. Minimal hardware requirements and instructions on how to run a full node can be found [here](/legacy/validator/run/mainnet).
