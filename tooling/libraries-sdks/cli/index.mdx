...
- [Running your own full node](/operate/operators/run-node)
...
The Celo CLI supports using a [Ledger hardware wallet](/tooling/wallets/ledger/setup) to sign transactions. Just add the `--useLedger` flag to a command that requires a signature.
