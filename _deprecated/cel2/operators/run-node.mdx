...
If you wish to migrate data from a Celo L1 node and have not yet done so, please see the [migration guide](/cel2/operators/migrate-node) before continuing. Alternatively, you can `snap` sync from scratch without migrating existing L1 data.
</Info>
...
2. Configure your `.env` file.
    ...
    Alternatively, you can start your node with `full` sync if you have a migrated L1 datadir. For instructions on obtaining a migrated L1 datadir, please see [Migrating an L1 Node](/cel2/operators/migrate-node).
...
1. A migrated full node datadir that has been synced to the migration block. See [Migrating an L1 Node](/cel2/operators/migrate-node) if you do not have this.
