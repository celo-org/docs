...
og:description: Documentation relevant to how to add currencies on Celo
---

The Celo protocol [supports paying](/legacy/protocol/transaction/erc20-transaction-fees) gas in tokens other than the native one, Celo. This document outlines requirements and processes to add a new gas token.
...
To enable a token as gas currency, two [governance proposals](/about-celo/protocol/governance/overview) need to be passed.
...
It would be a good consideration to update popular tooling (like [contractkit](/developer/contractkit)) before this proposal passes, so that most developers are ready to use the new gas as soon as it is enabled.
