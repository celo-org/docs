...
2. Configure your `.env` file.
    ...
    `op-reth` has no snap sync, so an archive node syncs by executing every block from genesis into an **empty** datadir. To skip the long initial sync, bootstrap from a published archive snapshot:

    ```text
    OP_RETH__SNAPSHOT=true
    ```

    With `NODE_TYPE=archive`, this downloads the archive snapshot. Leave `OP_RETH__SNAPSHOT=false` to execute every block from genesis instead. Either way, `DATADIR_PATH` must be empty on first start — a datadir written by `op-geth` cannot be reused.
