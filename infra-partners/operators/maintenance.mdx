...
If you need to rebuild a node, start from an empty `DATADIR_PATH` — a datadir written by op-geth cannot be reused. You can either bootstrap from a published snapshot (`OP_RETH__SNAPSHOT=true`) or execute every block from scratch; see [Run a node with Docker](/infra-partners/operators/run-node). Pre-L2 historical state for archive nodes is served separately — see [Running an archive node](/infra-partners/operators/archive-node).
