      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center">
        <div className="grid lg:grid-cols-2 gap-16 mb-16">
          <div className="flex flex-col justify-between">
            <div>
              <p className={`!font-futura !font-light text-6xl text-sand mb-4`}>
                Join the Celo ecosystem.
              </p>
              <p className="!font-futura !font-light text-xl text-slate-400 mt-3 mb-8">
                Build together with a global community of passionate developers.
              </p>
            </div>
            <div>
              <p className={`!font-futura !font-light text-4xl mb-6 max-w-md`}>
                Sign up for developer updates
              </p>
              <div className="flex flex-col gap-1 mb-16">
                <span className="!font-futura !font-light">Email*</span>
                <input
                  type="email"
                  placeholder="Enter your email"
                  className="px-3 py-2 bg-transparent border border-violet-800 border-solid rounded w-52 !font-futura !font-light"
                />
              </div>
            </div>
          </div>
            ...
            {communityLinks.map((link, index) => (
              <div className="flex flex-row w-full" key={index}>
                ...
                <div className="border border-violet-950 border-solid p-8 w-full">
                  <h3 className="!font-futura !font-light text-3xl mb-1">
                    {link.title}
                  </h3>
                  <p className="!font-futura !font-light text-white/60 !text-lg !mb-0">
                    {link.description}
                  </p>
                  <Button
                    size="lg"
                    className="mt-2 bg-violet-950 text-violet-400 hover:text-[#7C3AED]/80 !font-futura !font-light"
                  >
                    {link.action}
                  </Button>
                </div>
