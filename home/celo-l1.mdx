...
- **On-chain randomness** — the `Random` contract was removed; contracts use the `PREVRANDAO` opcode instead. See [Deactivated Random Contract](/operate/specification/l2-migration#deactivated-random-contract).
...
- **The `celo-blockchain` client** — the L1 client (docker image `us.gcr.io/celo-org/geth`) stopped syncing new blocks at the migration. To serve pre-migration state, run a [Historical RPC Service](/operate/operators/archive-node).
...
| L2 migration changes in the specs | [/operate/specification/l2-migration](/operate/specification/l2-migration) |
...
- [Node operators](/operate/operators/overview) - running Celo L2 nodes today
