...
sidebarTitle: "Release Gold"
og:description: Introduction to ReleaseGold including examples, use cases, and FAQ.
---
...
<AccordionGroup>
<Accordion title="Can I vote for validators, or run a validator using my `ReleaseGold` CELO balance?">
...
<Accordion title="Can the `releaseOwner` access my CELO?">
...
<Accordion title="Can I move the CELO released by the `ReleaseGold` contract to another address?">

- Of course! Once CELO is released and the cliff has passed, the beneficiary is free to do what they want with it.
</Accordion>

<Accordion title="Why do I need to authorize separate keys for voting and validating? Can’t I do it using the private key for my beneficiary address?">
...
<Accordion title="Can I change the beneficiary?">
...
<Accordion title="What if I lose the private key associated with the beneficiary address?">
...
<Accordion title="What happens if there is a bug in the `ReleaseGold` contract?">
...
<Accordion title="What is the distribution ratio?">
