...
- You have your private key held on a [Ledger Nano S or Ledger Nano X](/tooling/wallets/ledger/setup) device, and you have a second such device available for managing a voting key. If you only have a single Ledger available, see [below](#using-a-single-ledger).
...
Please refer to the [Ledger Troubleshooting](/tooling/wallets/ledger/to-celo-cli#troubleshooting) for issues using Ledgers with the Celo CLI.
...
- Use that Locked CELO to vote for Validator Groups to operate Celo's [staking](/home/protocol/staking/index) mechanism (and in doing so be ready to receive [epoch rewards](/home/protocol/epoch-rewards/index))
...
As a first step, follow [these instructions](/tooling/wallets/ledger/setup) for both Ledgers to install the Ledger Celo app, obtain and verify the associated addresses, and (recommended) run a test transaction on the Celo Sepolia test network.

<Note>
The latest version of the Celo Ledger app is 1.1.8. If you are already using a Ledger with an earlier version installed, please [upgrade](/tooling/wallets/ledger/setup).
</Note>

The remainder of this guide assumes you are using the first address available on each Ledger. You can add the flags described in [these instructions](/tooling/wallets/ledger/setup) to commands below to use different addresses.
...
First, read [these instructions](/tooling/wallets/ledger/setup) carefully. Then, wherever you see instructions to connect your Vote Signer Ledger, for each command line containing `--useLedger` also add `--ledgerCustomAddresses "[1]"`. If in doubt, [ask for help](#support).
...
- **Local Celo Blockchain node**: You can run a full node on your local machine which will communicate
  ...
  To do this, follow the tutorial for [running a full node](/operate/operators/run-node) (and make sure to pass `--usb`).
...
To allow you to keep your Beneficiary Ledger offline on a day-to-day basis, it’s recommended to use a separate [Authorized Vote Signer Account](/home/protocol/staking/key-management/detailed#authorized-vote-signers) that will vote on behalf of the beneficiary.
...
For more details, check out the [Voting for Validators page](/home/protocol/staking/voting), which contains useful background on how voting Validator Elections work, as well as more guidance on how to select a Validator Group to vote for. For now, all you need to know is that:
...
Keeping this in mind, you will need to find a Validator Group to vote for and copy its address. You can find this information on community [validator explorers](/home/protocol/staking/voting#validator-explorers) such as [Celo Mondo](https://mondo.celo.org/) and [Bi23 Labs' `thecelo` dashboard](https://thecelo.com).
...
You might want to read more about [choosing a Validator Group](/home/protocol/staking/voting) to vote for, and how [voter rewards](/home/protocol/epoch-rewards/index) are calculated. You can vote for up to ten different Groups from a single account.
...
You can also read more about how Celo's [staking](/home/protocol/staking/index) and on-chain [Governance](/home/protocol/governance/overview) mechanisms work.
