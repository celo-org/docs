...
A `$NETWORK-$PREVIOUS_RELEASE-libraries.json` file is written to disk describing linked library addresses. `release:make` requires it.
...
PREVIOUS_RELEASE="core-contracts.v$((N-1))"
RELEASE_CANDIDATE="core-contracts.v${N}"
PRIVATE_KEY=                        # deployer key for $NETWORK
