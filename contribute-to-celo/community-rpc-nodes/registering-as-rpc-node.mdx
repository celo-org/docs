...
description: Step-by-step instructions for registering a community RPC provider on chain to be eligible for rewards
---
...
<Note>
You can use the Celo CLI with a Ledger hardware wallet (see [CLI docs](/tooling/wallets/ledger/to-celo-cli)) or pass a private key directly with the `--privateKey` flag. Both options work with any transaction-signing command.
</Note>
