...
2. Configure your `.env` file.
    ...
    **Datadirs from op-geth cannot be reused**
    ...
    Your node will run as a `full` node by default, but can also be configured as an `archive` node if you wish to preserve access to all historical state. See [Running an archive node](/operate/operators/archive-node) for more information.
