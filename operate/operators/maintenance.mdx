...
description: Routine upgrade and maintenance tasks for a running Celo L2 node
---
...
Network hardforks require running a compatible client version *before* the fork block, or your node will stop following the chain. Watch the [Notices](/operate/notices/archive/l2-migration) for each upgrade's required versions and timing. The next major required migration is the move from op-geth to op-reth — see [End of Support for op-geth](/operate/notices/op-geth-deprecation).
</Warning>
