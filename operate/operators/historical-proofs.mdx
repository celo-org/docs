...
description: Configure op-reth to serve deep eth_getProof requests without running a full archive node
---
...
here. See [End of Support for op-geth](/operate/notices/op-geth-deprecation)
...
1. Follow [Run a node with Docker](/operate/operators/run-node) to get a
...
If [monitoring](/operate/operators/monitoring) is enabled, the same window
...
- [Running an archive node](/operate/operators/archive-node): the
