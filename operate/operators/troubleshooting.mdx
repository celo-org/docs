...
description: Fixes for common Celo node problems, from stalled transactions to sync failures
---
...
If you are hosting a public RPC node, see [Run a public RPC node](/operate/operators/public-rpc-node) for serving transaction lookups by hash.
...
See [Monitoring & metrics](/operate/operators/monitoring) for the Grafana dashboard and the metrics that show sync health.

## Node Is Not Syncing or Has No Peers

If your node stalls or falls behind the sequencer, it usually has too few peers. Check `op_node_default_peer_count` (see [Monitoring & metrics](/operate/operators/monitoring)); if it is low or zero, your node cannot discover or reach other nodes. This is almost always a P2P configuration problem:
