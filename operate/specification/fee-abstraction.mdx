...
description: How Celo lets users pay gas in ERC20 tokens instead of native CELO
---
...
A request pays fees in native CELO only when `feeCurrency` is omitted or `null`. The zero address is not an alias for native CELO; the node rejects it as an unregistered fee currency.
...
Transactions with unregistered `feeCurrency` values are not accepted into the tx pool and dropped from the pool if they have been previously accepted. The same is true if the `maxFeePerGas` falls below the current base fee. Only an absent `feeCurrency` (an empty RLP string) means native CELO; a CIP-64 transaction whose `feeCurrency` is the zero address is invalid, and so is any block that contains one.
