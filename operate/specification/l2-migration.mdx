...
description: The changes introduced by the L1 to L2 migration that tool developers and operators need to know
---
...
- [Removed precompiles](/operate/specification/smart-contract-updates-from-l1#precompiles-deprecation) (all except the `transfer` precompile)
...
Read more on new [epoch management and reward distribution](/operate/specification/smart-contract-updates-from-l1#epochs-and-rewards) or [deprecated precompiles](/operate/specification/smart-contract-updates-from-l1#precompiles-deprecation).
...
New transactions can't be submitted using the following transaction types, see also the [tx types page](/operate/specification/transaction-types):
...
The `CeloUnreleasedTreasury` is a new contract available on the migrated Celo L2. See [the spec](/operate/specification/smart-contract-updates-from-l1#celo-minting) for more information.
