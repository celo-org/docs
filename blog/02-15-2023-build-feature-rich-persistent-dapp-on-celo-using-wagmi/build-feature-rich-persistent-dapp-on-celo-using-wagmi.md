...
- `components/App/index.tsx`
  ...
  - When `errorMessage` is updated, the `useEffect` hook is triggered to replace the `message` with `balance` variable. 
  ...
  - `handleClick()` is configured to dynamically select and run contract functions in the `components/Api/index.ts` based on the selected function name. `sendTransaction()` is invoked in this child component, extract the provider object, and parsed to `sendTransaction()`.
