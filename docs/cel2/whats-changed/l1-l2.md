...
This is a temporary situation, and we will be working on re-introducing active roles for validators after Mainnet launch. For now, validators do not need to run any special L2 infrastructure.
...
The amount of rewards to be distributed has not been decided. However, rewards will likely be lower than in the Celo L1 during the period of time that validators are not required to run infrastructure.
...
All Celo specific precompiles will be removed except for the transfer precompile which supports Celo [token duality](https://specs.celo.org/token_duality.html) (the native asset CELO is also an ERC20 token)

## Randomness

The random contract will be removed. If randomness is needed then the PREVRANDAO opcode can be used. See [here](https://specs.celo.org/l2_migration.html#deactivated-random-contract) for more details.

## Blocks

* Block interval will change from 5s to 1s
* Block gas will change from 50m to 30m
