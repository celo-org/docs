...
description: Learn how to run a full Celo Node
---


# Running a Celo Node

This guide is designed to help node operators run a Celo L2 node, and assumes you have already migrated data from a Celo L1 node or plan to `snap` sync from scratch. If you wish to migrate data from a Celo L1 node and have not yet done so, please see the [migration guide](/cel2/operators/migrate-node.md) before continuing.
...
2. Configure your `.env` file.
    ...
    Alternatively, you can start your node with `full` sync if you have a migrated L1 datadir. For instructions on obtaining a migrated L1 datadir, please see [Migrating an L1 Node](/cel2/operators/migrate-node.md).
...
1. A migrated full node datadir that has been synced to the migration block. See [Migrating an L1 Node](/cel2/operators/migrate-node.md) if you do not have this.
