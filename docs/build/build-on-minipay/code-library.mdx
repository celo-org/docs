...
On this page you can find snippets of code that can be used to implement flows inside MiniPay.

<ColoredText>UI/ UX</ColoredText>
- [Get Users Account Address](#get-the-connected-users-address-without-any-library)
- [Check the cUSD Balance of an Address](#check-cusd-balance-of-an-address)
- [Check if a Transaction Succeeded](#check-if-a-transaction-succeeded)
- [Resolve MiniPay phone numbers to Addresses](#resolve-minipay-phone-numbers-to-addresses)

<ColoredText>PayLinks</ColoredText>
- [Request an ERC20 token transfer](#request-an-erc20-token-transfer)

<ColoredText>Fees</ColoredText>
- [Estimate Gas for a Transaction in CELO](#estimate-gas-for-a-transaction-in-celo)
- [Estimate Gas for a Transaction in cUSD](#estimate-gas-for-a-transaction-in-cusd)
- [Calculate cUSD to be spent for transaction fees](#calculate-cusd-to-be-spent-for-transaction-fees)
...
```js
// The code must run in a browser environment and not in a node environment
if (window && window.ethereum) {
  // User has an injected wallet
